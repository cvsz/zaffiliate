...
- `npm test`: **695 tests — 687 pass, 0 fail, 8 skips** (2026-09-22, verified after dependabot #56/#57/#58 merges on pristine `npm ci`; skips are environment-gated integrations requiring live Postgres/optional backends). Production readiness gates fully executed: `./scripts/verify.sh` ALL GATES GREEN, `./scripts/security-check.sh` PASS, load/soak/fault-inject all PASS, backup-restore-drill executed (schema.sql 112KB), restore-rehearsal PASSED (39 tables, 13 RLS+forced, cross-tenant isolation, golden publication flow, metrics CTR=CVR=10%).
