...
- `npm test`: **735 tests — 735 pass, 0 fail, 8 skips** (2026-09-22; 8 skips are DB-reachability-gated integrations that run green against live Postgres: `shopee-th-import-repo.integration.test.js` RLS+idempotency, `shopee-th-import-repo.integration.test.js` offer access under app role; remaining skips are environment-specific). Production readiness gates fully executed: `./scripts/verify.sh` ALL GATES GREEN, `./scripts/security-check.sh` PASS, load/soak/fault-inject all PASS, backup-restore-drill executed (schema.sql 112KB), restore-rehearsal PASSED (39 tables, 13 RLS+forced, cross-tenant isolation, golden publication flow, metrics CTR=CVR=10%).
