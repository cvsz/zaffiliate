...
## Backlog (dependency-ordered, now trimmed)

video factory FFmpeg render (DEFERRED until B7 writable) · image factory media persistence (B7) · warehouse/OLAP + attribution windows/confidence (intentionally deferred) · workers/scheduler split apps (stub-class, `NOT_REQUIRED_FOR_RELEASE`) · trained models INTEL-3+ (awaiting production data + shadow paired data) · calendar API/UI (still MISSING, low priority) · product variant/promotion/refund/payout deep contracts (backlog). Previously backlog items now **closed**: publishing HTTP, trend/opportunity engine, automation durable state, k8s minimal, MM-007 policy registry, Mission Control remaining panels, ingress remaining emitters, analytics re-export, CodeQL/dependabot, error-model consolidation.
