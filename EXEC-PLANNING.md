...
| AFF-005 PostgreSQL foundation | app-level DB access | pooled `packages/db/src/client.js` (ipv4first TLS, `pg` lazy, credential-redacted `check()`), 12 migrations 001–012 drift-checked, 31 tables RLS/FORCE, self-host `postgres:17` healthy (31 tables verified) | COMPLETE | reuse |
| AFF-006 Database migrations | programmatic migrator | checksummed `packages/db/src/migrator.js` (sorted, pending/applied/drift, per-migration transaction unwrapping BEGIN/COMMIT, fail-closed on drift, `db/migrations/ROLLBACK.md` 007–012) + `packages/db/src/cli.js` JSON CLI, `compose.selfhost.yaml` hard-gated migrator | COMPLETE | reuse |
...
| AFF-014 Object storage | media assets | `packages/storage/src/{index,content-validation,s3}.js` (MIME allowlist, size caps, traversal-proof `tenants/<uuid>/…` keys, HMAC-signed URLs, SigV4 four-stage HMAC, sharing immutable-key contract; writes 403-gated B7 provider-side) + `packages/storage/src/content-validation.js` hardening | PARTIAL | extend (B7 bucket perms) |
...
### Milestones vs repo reality (2026-08-30)

M0 Secure Foundation: **COMPLETE** (app DB client `packages/db` 12 migrations RLS 31 tables, Redis declared runtime + Lua limiter, storage hardened local+SigV4 with 403-gated writes, 152 check gates clean). M1 Affiliate MVP: **COMPLETE** (products→offers→links→clicks→conversions→margins durable via 007 + campaign lifecycle 011 + reconciliation 012, outbox dispatcher, `/go` + webhook durable attribution, campaign-scoped links). M2 Content Factory: **PARTIAL** (template-driven runtime complete with provenance; FFmpeg render + media persistence still deferred on storage creds). M3 Publisher: ~75% (TikTok/Shopee strong, publishing boundary + durable jobs + outbound idempotency; Meta/YT still thin). M4 Revenue Intelligence: **~85%** (analytics envelope/dedupe/persist, commerce stale-claim BLOCK, intelligence INTEL-0..2 complete; warehouse/attribution windows remain). M5–M7: open (cutover/retirement remain maintainer-gated).
