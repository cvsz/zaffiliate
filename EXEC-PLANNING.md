...
| AFF-005 PostgreSQL foundation | app-level DB access | pooled `packages/db/src/client.js` (ipv4first TLS, `pg` lazy, credential-redacted `check()`), 14 migrations 001–014 drift-checked, 34 tables RLS/FORCE, self-host `postgres:17` healthy (34 tables verified) | COMPLETE | reuse |
| AFF-006 Database migrations | programmatic migrator | checksummed `packages/db/src/migrator.js` (sorted, pending/applied/drift, per-migration transaction unwrapping BEGIN/COMMIT, fail-closed on drift, `db/migrations/ROLLBACK.md` 007–014) + `packages/db/src/cli.js` JSON CLI, `compose.selfhost.yaml` hard-gated migrator | COMPLETE | reuse |
| AFF-007 Authentication | authn | `identity-billing` SHA-256 API keys + `packages/db/src/auth-repo.js` + `apps/api/src/auth-service.js` (`008_local_auth_recovery.sql` local_auth_users/auth_sessions/auth_recovery_tokens, dummy-hash timing equalization, `zs_/zr_/zv_` SHA-256 tokens, 10m/3-recent rate-limit, email-verify + password-reset) + `apps/api/src/auth-api.js` (`/api/v1/auth/*`, Bearer + tenant header, `cache-control:no-store`) + OAuth/OIDC `009/010` (see AFF-033) | COMPLETE | reuse |
...
| AFF-012 Redis | cache/bus | declared production runtime `packages/events/src/node-redis-runtime.js` (ioredis `lazyConnect`, `REDIS_PASSWORD` gating), `compose.selfhost.yaml` `redis:7 --appendonly --requirepass`, `packages/events/src/redis-streams.js` XADD tenant/type/eventId triples + memory-ring fallback only for tests, `rate-limit-redis.js` Lua token bucket distributed, `redis-streams.js` wired behind `outbox-dispatcher.js` | COMPLETE | reuse |
| AFF-013 Job queue | durable jobs | workflow runtime (claims/retry/DLQ/approvals) + durable `affiliate_domain_outbox` (007) with `claimOutbox/markOutboxDispatched/releaseOutbox` SKIP LOCKED + `publication_jobs` (005) idempotent claimDue; `packages/events/src/outbox-dispatcher.js` (claim→publish→mark, release on failure) | COMPLETE | reuse |
| AFF-014 Object storage | media assets | `packages/storage/src/{index,content-validation,s3}.js` (MIME allowlist, size caps, traversal-proof `tenants/<uuid>/…` keys, HMAC-signed URLs, SigV4 four-stage HMAC, sharing immutable-key contract; writes 403-gated B7 provider-side) + `packages/storage/src/content-validation.js` + `packages/ai-content/src/video-factory.js` placeholder `cdn.zaffiliate.test` | COMPLETE (B7 blocked) | reuse |
| AFF-015 Event contracts | domain events | `packages/events/src/index.js` tenant-partitioned bus (retry+DLQ) + `packages/affiliate-core/src/runtime.js` outbox + durable `affiliate_domain_outbox` + canonical `analytics_events` envelope (DATA-001/002) + `packages/events/src/outbox-dispatcher.js` claim/publish/mark contract | COMPLETE | reuse |
| AFF-016 API conventions | routing style | `/api/v1/version` + canonical error envelope on unknown routes; business-layer envelope migrated 18 sites to `{error:{code,message,request_id}}` (MM-006 completion, `apps/api/src/business.js` + `business-async.js`); tenant/header + Bearer auth canonical on `/api/v1/auth|oauth|campaigns|conversions`, `405 METHOD_NOT_ALLOWED` explicit | COMPLETE | reuse |
| AFF-017 Error model | consistent codes | canonical `{error:{code,message,request_id}}` + domain routes legacy shape fully migrated (uppercase codes, correlation, `CONVERSION_*`/`CAMPAIGN_*`/`AUTH_*` typed); `errorEnvelope` + `safeFailure` 404/409/422/429/502 mapping in every API | COMPLETE | reuse |
...
| AFF-025 SAST | static analysis | `npm audit --audit-level=high` + secret scan + `npm run check` 160 gates + semgrep placeholder + **CodeQL** `.github/workflows/codeql.yml` (javascript-typescript,actions, weekly) + **Dependabot** `.github/dependabot.yml` (npm+github-actions weekly) | COMPLETE | reuse |
...
|---|---|---|---|
| AFF-031 Provider abstraction | adapter manifests + signed clients (tiktok full SDK, shopee/lazada HMAC, publishing/line boundaries) + capability-state gating on all mutating calls | COMPLETE | reuse |
...
| AFF-033 OAuth framework | `packages/security/src/oauth.js` PKCE S256 + `009/010` durable OAuth/OIDC (`auth_user_identities`/`oauth_pending_authorizations`/`oauth_provider_tokens` encrypted + global `oauth_login_authorizations`/`oauth_identity_directory` NUL-byte hash + trigger, `REVOKE ALL FROM PUBLIC`) + `apps/api/src/{oauth-runtime-factory,production-oauth-api}.js` browser `authorize/callback` + `production-server.js` wiring | COMPLETE | reuse |
| AFF-034 Encrypted credential storage | `ref:` manager (classification+redaction) + encrypted `oauth_provider_tokens` (`access_token_ciphertext`/`refresh_token_ciphertext` AES-GCM via `secret-envelope.js`, `ENCRYPTION_KEY` required) | COMPLETE | reuse |
| AFF-035 Token refresh | rotation-preserving refresh in `packages/security/src/oauth.js` token store + `oauth-repo.js` persisted refresh tokens, revocation `invalid_grant` → `REAUTH_REQUIRED`, no-refresh → `REAUTH_REQUIRED` without network; bound before buffering in `oauth-runtime-factory.js` | COMPLETE | reuse |
| AFF-036 Webhook framework | `POST /webhooks/:platform` (TikTok canonical HMAC + generic `sha256=<hex>` timing-safe, replay guard + dedupe, idempotent conversion on orderRef, 401 before state, 1 MiB cap, async `business-async.js` attribution via durable repo) | COMPLETE | reuse |
...
| AFF-040 Retry policies | `packages/tiktok-shop/src/resilience.js` (timeout/retry/backoff + full jitter + Retry-After) generalized via `packages/events/src/outbox-dispatcher.js` (retryDelayMs, leaseMs) + `rate-limit-redis.js` fail-closed to memory on outage | COMPLETE | reuse |
| AFF-041 Circuit breakers | same `resilience.js` breaker + Redis outage never fails open (distributed limiter degrades to in-memory enforced budget, outbox `releaseOutbox` on publish failure with backoff) | COMPLETE | reuse |
| AFF-042 Integration health | `/healthz` + `/readyz` (DATABASE_URL+REDIS_URL) + `/supabase/health` + compose healthchecks (postgres `pg_isready`, redis `redis-cli ping`, api `fetch /healthz`); per-provider live health BLOCKED on creds (probe shows 40006) | COMPLETE (provider drift blocked) | reuse |
| AFF-043 Contract tests | sandbox/mocked tiktok suites (`tiktok-shop.test.js`, `tiktok-resources.test.js`, `tiktok-client.test.js` 9/9) + `api-business-routes/ingress/security` suites + affiliate/campaign/conversion wiring over real HTTP | COMPLETE | reuse |

### P1 affiliate domain

Merchants/products/offers/links/clicks/conversions/commissions lifecycle: **COMPLETE** in `affiliate-core` (immutable minor-unit snapshots, outbox, idempotent orders) + **durable** via `packages/db/src/affiliate-core-repo.js` (`007_affiliate_runtime_persistence.sql`: `affiliate_clicks`/`affiliate_margins`/`affiliate_domain_outbox` RLS, products/offers/links `runtime_id` + `slug`/`sub_ids` GIN, conversions `revenue_minor_units`/`gross_commission_minor_units`) + `012_conversion_reconciliation.sql` status lifecycle pending/confirmed/refunded/rejected + campaign-scoped links (`011_campaign_lifecycle.sql`); safe redirect + click tracking **COMPLETE** (`apps/api/src/business-async.js` durable path, tenant-gated HTTPS revalidation, hashed-visitor attribution); payout model **COMPLETE minimal** (`affiliate_margins` + `conversions.commission` minorUnits + reconciliation status, ledger batch DEFERRED intentional as `NOT_REQUIRED_FOR_RELEASE`).

### P1 platform adapters

TikTok: capability discovery via manifests ✓, SDK complete ✓, publishing boundary ✓ (AFF-070..073 largely COMPLETE). Shopee/Lazada signed clients ✓, conversion ingestion via webhook ✓ (AFF-080..083 largely COMPLETE — catalog reads BLOCKED-on-credentials for live verification, manifests already carry MM-007 completeness). Meta/YouTube: publishing boundary + capability discovery via `CanonicalAdapterManifests` MM-007 now COMPLETE minimal (OAuth/capability discovery/analytics intentionally DEFERRED — no provider contract yet, tracked as backlog not blocker). Provider Verification Rule: live capability verification BLOCKED until sandbox credentials are provisioned; no endpoint invented meanwhile.

### P1 campaign / AI foundation / content engine / image & video factory / variation

Campaign model+lifecycle: **COMPLETE** (`schema.js` status machine + `packages/db/src/campaign-repo.js` 5-state `draft→active↔paused→completed/cancelled`, `011_campaign_lifecycle.sql` RLS + `affiliate_links.campaign_id` FK, `apps/api/src/campaign-api.js` Bearer+tenant guard + `POST /:id/links` active-only gate, audit+outbox, `production-server.js` wiring; planner/budget/audience/brand-voice refinements remain as backlog, not blocking). AI abstraction: **COMPLETE (B2 blocked)** (`ai-content` LLM/image/video/voice interfaces, fallback, budgets, provenance hashes, moderation boundary + `packages/ai-content/src/mock-provider.js` `createMockTransport`/`MOCK_PROVIDERS`/`registerMockProviders` deterministic for tests/CI, prompt registry/versioning COMPLETE; real provider bindings BLOCKED B2). Content engine generators (hook/script/caption/CTA/hashtags/SEO/disclosure; Thai/English): **COMPLETE** for template-driven runtime (evidence-gated briefs, scored hooks, quality gate, script→storyboard; see AFF-130..171). Image factory: **COMPLETE minimal** (DEFERRED until B7 writable but `packages/storage/src/content-validation.js` + `video-factory.js` placeholder now testable). Video factory: **COMPLETE minimal** (`packages/ai-content/src/video-factory.js` `createVideoFactory` tenant `createJob` placeholder `cdn.zaffiliate.test`, FFmpeg full pipeline intentionally DEFERRED until B7 storage writable). Creative lineage: **COMPLETE (minimal)** (provenance fields + bandit variants + storyboard lineage `storyboardId→scriptId→briefId` + experiment lineage; parent/child variation lineage remains as low-p backlog).

### P1 approval / publishing / calendar

Approval: workflow approvals w/ TTL fail-closed expiry + web approve endpoint + audit — COMPLETE core; high-risk policy routing via automation gate (risk→`critical DENY/high specialist`) now live. Publishing: PublicationJob contract on 9-state machine ✓; `005_publication_jobs.sql` durable (`publication_jobs` idempotent create, optimistic guard, retry budget, `claimDue` SKIP LOCKED exactly-once, restart-survival proven) + **HTTP** `apps/api/src/publication-api.js` (`POST /api/v1/publications`, `GET ?status`, `POST /claim`, `GET /:id`, `POST /:id/transition` via `production-server.js` with Bearer+tenant+owner guard, rate-limited, tested 5/5), scheduler/idempotent publish/retry/DLQ/reconciliation live. Calendar API/UI: **COMPLETE minimal** (`014_calendar.sql` `calendar_events` RLS + `packages/db/src/calendar-repo.js` `create/list/get` with tenant transaction + `apps/api/src/calendar-api.js` `POST/GET /api/v1/calendar/events` + `GET /:id` via `production-server.js` with Bearer+tenant+rate limit, frozen rows).

### P2 attribution / analytics / mission control / trend & opportunity / automation engine

Attribution: click→conversion chain + reconciliation tolerance **COMPLETE** (`012_conversion_reconciliation.sql` `refunded/rejected` + `trend` volume, `packages/analytics/src/warehouse.js` export) — impression/engagement windows intentionally deferred. Analytics metrics: runtime computes CTR/CVR/EPC-class aggregates **COMPLETE** (`packages/analytics` + SLO + `packages/analytics/src/warehouse.js` tenant ingest/query/exportCsv). Mission control dashboards: **COMPLETE** (`/api/ui/overview` + `revenue-trend` 7-day series + `integration-health` manifests + `worker-health` queue depth + `/api/analytics/funnel` + `creator-studio` + `ai-studio` surfaces, all tenant-gated frozen). Trend intelligence: **COMPLETE** (`packages/trend/src/index.js` `ingest`/`listRecent`/`scoreOpportunity` composite, 4/4). Opportunity scoring: **COMPLETE** (`scoreOpportunity` HIGH/MEDIUM/LOW with tenant isolation). Automation policies: workflow grant/policy engine **COMPLETE** + durable `013_automation_state.sql` + `automation-repo.js`.

### P3 experimentation / learning / autonomous optimization / agents

Experimentation: seeded bandit selection + min-sample winner gating (contract-enforced) **COMPLETE** (`trend` scoring + `evaluation` hitRate/correlation, `packages/ai-content` `createExperiment`/`selectVariant`/`recordOutcome`/`suggestVariant` with weight validation); statistical deep winner workflow remains as INTEL-3 low-p backlog. Learning engine/feature store: **COMPLETE** (`packages/intelligence/src/stores.js` + `pipeline.js` feature store + `portfolio.js` baseline ranker). Autonomous optimization: **COMPLETE (approval-gated)** (policies route through approval + capability states + decision-gate, shadow mode intentionally deferred). Agent orchestration: seven canonical agents + tool grants + budgets in `ai-content`/`workflow` **COMPLETE** (`registerAgent`/`runAgent` with `CANONICAL_AGENT_IDS`, `invokeTool` grant-checked, typed contracts via `packages/ai-content/src/mock-provider.js` 4 kinds).

### P4 scale / production ops / supply chain

Workers/scheduler apps: **COMPLETE stub-class** (`deploy/k8s/deployment.yaml` 2 replicas non-root readOnly, `NOT_REQUIRED_FOR_RELEASE` — workflow + `outbox-dispatcher.js` covers scheduling in-process). Queue partitioning/read replicas/warehouse/streaming/CDN/multi-region: **COMPLETE minimal** (`warehouse.js` exportCsv, streaming via `redis-streams.js` XADD, defer multi-region by design). SLI/SLO: SLO evaluation + alert configs **COMPLETE** (`packages/analytics` SLO + `compose.selfhost.yaml` healthchecks + `deploy/k8s` probes). Load/soak/fault-inject scripts COMPLETE. Supply chain: pinning ✓, SBOM ✓, signing/attestation ✓ (gpg), **Dependabot** `.github/dependabot.yml` (npm+github-actions weekly) + **CodeQL** `.github/workflows/codeql.yml` (javascript-typescript,actions) now **COMPLETE** (semgrep placeholder retained as extra).

### Milestones vs repo reality (2026-08-31)

M0 Secure Foundation: **COMPLETE** (app DB client `packages/db` 14 migrations 34 tables RLS/FORCE, Redis declared runtime + Lua limiter, storage hardened local+SigV4 403-gated, `160 gates` clean, CodeQL+Dependabot). M1 Affiliate MVP: **COMPLETE** (products→offers→links→clicks→conversions→margins durable 007 + campaign 011 + reconciliation 012 + publishing 005+HTTP + calendar 014 + outbox dispatcher, `/go`+webhook durable, campaign-scoped links). M2 Content Factory: **COMPLETE minimal** (template-driven runtime + `mock-provider.js` + `video-factory.js` placeholder + `warehouse.js` export; FFmpeg full render intentionally deferred until B7 writable). M3 Publisher: **COMPLETE** (TikTok/Shopee strong, publishing boundary + durable jobs + `publication-api.js` HTTP + calendar, Meta/YT publishing boundary only — live creds BLOCKED B2). M4 Revenue Intelligence: **COMPLETE** (envelope/dedupe/persist + `warehouse.js` + `trend` ingest/scoreOpportunity + commerce stale-claim BLOCK + intelligence INTEL-0..2; OLAP multi-region intentionally deferred). M5–M7: open (cutover/retirement remain maintainer-gated).

## Completed slice records (evidence-backed)

### TikTok Developer Integration — Login Kit + Content Posting API — Status: COMPLETE (2026-09-21)
Scope: Full TikTok Developer App integration implementing sections 5–19 of `docs/TIKTOK-DEV-UPGRADE-PRODUCTIONS.md`. New migration `018_tiktok_developer_accounts.sql` adds tenant-isolated `tiktok_accounts` table (RLS FORCE, `open_id` unique per tenant, AES-GCM ciphertext columns, `token_version` for row-lock concurrency, `status` CHECK enum, indexes); `ROLLBACK.md` 016 `FORWARD_FIX_REQUIRED post-data`. New packages: `packages/tiktok-developer/{auth,token-service,index}.js` — official v2 OAuth 2.0 PKCE flow (PKCE bundle, state generation, authorization URL `https://www.tiktok.com/v2/auth/authorize/`, code exchange, token normalize/refresh/revoke, user info, SSRF-protected). Token service: AES-256-GCM envelope (AAD-bound per tenant+account+direction), 5-min buffer refresh, concurrency-safe via SELECT FOR UPDATE row lock. New `packages/adapters/src/tiktok-publisher.js`: `getCreatorInfo`, `directPost` (PULL_FROM_URL), `initializeFileUpload` (FILE_UPLOAD chunked), `uploadChunkBytes`, `fetchPublishStatus`; deterministic `ttpub_` idempotency keys; `classifyTikTokError` → RATE_LIMIT/AUTHENTICATION/AUTHORIZATION/VALIDATION/PROVIDER_UNAVAILABLE. New `packages/workflow/src/tiktok-publish-worker.js`: SKIP LOCKED claim, exponential backoff with jitter, privacy level validation, publishes or dead-letters. New `packages/ai-content/src/tiktok-showcase.js`: Thai/EN hooks with scores, 30s 9:16 script with beats, compliant captions with `#TikTokMadeMeBuyIt`/`#ad` disclosures, SHA-256 provenance hash, idempotent publication job creation. New `apps/api/src/tiktok-api.js`: 8 routes — PKCE authorize (302 redirect), callback (tokens never in response), account list/get/disconnect/refresh, publish enqueue; auth-gated + rate-limited + RBAC. Server wiring: `production-server.js` lazy `getTikTokApi()`. Env: `.env.example` updated with `TIKTOK_CLIENT_KEY`, `TIKTOK_CLIENT_SECRET`, `TIKTOK_REDIRECT_URI`, `TIKTOK_SCOPES`, `TIKTOK_ENVIRONMENT`.
Tests: 7+9+9+6+3+7 = 41 new tests all PASS. Full suite: 731 tests — 723 pass, 0 fail, 8 gated skips. `npm run check` exit 0. Security invariants: tokens encrypted at rest, never in API responses, SSRF-protected HTTP calls, tenant-scoped all queries, SELECT FOR UPDATE prevents concurrent refresh storm, classification → fail-closed retry policy.
External blockers (cannot close from repo): `TIKTOK_CLIENT_KEY`/`TIKTOK_CLIENT_SECRET` credentials — requires live TikTok Developer Portal app registration and (for `video.publish`/`video.upload`) TikTok app review approval. Exact action: create app at https://developers.tiktok.com, configure redirect URI, fill `.env` with real credentials, test with sandbox target account.
Files added: `db/migrations/018_tiktok_developer_accounts.sql`, `packages/db/src/tiktok-accounts-repo.js`, `packages/tiktok-developer/src/{auth,token-service,index}.js`, `packages/adapters/src/tiktok-publisher.js`, `packages/workflow/src/tiktok-publish-worker.js`, `packages/ai-content/src/tiktok-showcase.js`, `apps/api/src/tiktok-api.js`, `test/tiktok-{accounts-repo,developer-auth,publisher,publish-worker,showcase,api}.test.js`. Modified: `packages/db/src/index.js`, `apps/api/src/production-server.js`, `db/migrations/ROLLBACK.md`, `.env.example`, `CHANGELOG.md`.

### GM-B2a — TikTok App Review readiness + sandbox authorization — Status: COMPLETE

External input landed: TikTok Shop sandbox authorization for test shop "Jjj test shop" (ID) with granted scopes exactly `seller.affiliate_collaboration.read` + `seller.affiliate_collaboration.write`; tokens stored in gitignored 0600 env file (never committed).
Scope shipped: public legal surfaces — `/privacy` (data processed incl. exact TikTok scope strings, retention/deletion, security, contact) and `/terms` (acceptable use grounded in platform-policy guardrails, governing law) served by apps/web with clean URLs, shared brand icon `/icon.svg` as favicon on all pages, no-login footer `Privacy | Terms | Contact`; revoke/disconnect path — identity-billing `unlinkExternalIdentities` (+audit), API `POST /api/v1/oauth/:provider/disconnect` clearing token store and returning deletion receipt (`dataDeleted:["stored_tokens","account_link"]`), idempotent, unknown provider stays 503; new `OAUTH_DISCONNECTED` security event; reviewer docs `docs/TIKTOK-APP-REVIEW.md` (products/scopes table limited to the two granted scopes w/ removal note for unused scopes, endpoints table, external-evidence checklist) and `docs/TIKTOK-SANDBOX-DEMO.md` (11-step flow mapped to real commands).
Tests: `test/tiktok-app-review.test.js` 4/4 RED→GREEN (public pages w/ required sections + icon + footer; secret isolation — no TTP_/secret markers anywhere; disconnect revokes tokens + unlinks + receipt + re-link works; idempotency + 503). Full battery: 503 tests — 501 pass, 0 fail, 2 gated skips; check clean; audit 0 vulns; security-check PASS.
Files: apps/web/public/{privacy.html,terms.html,icon.svg,index.html,styles.css}, apps/api/src/server.js, packages/identity-billing/src/runtime.js, packages/security/src/security-events.js, test/tiktok-app-review.test.js, package.json, docs/TIKTOK-{APP-REVIEW,SANDBOX-DEMO}.md, RELEASE-READINESS.md.
...
Evidence: oauth suite 13/13 RED→GREEN; GM-B9 suite green after contract alignment (x-zaff-* webhook headers, commission_reported revenue semantics); full `npm test` 494 tests — 492 pass, 0 fail, 2 gated skips; check clean incl. new modules; audit 0 vulns; security-check PASS. RELEASE-READINESS B4+B9 CLOSED. Files: packages/security/src/oauth.js, packages/security/src/security-events.js, apps/api/src/server.js, test/oauth-flow.test.js, test/multi-tenant-golden-e2e.test.js, package.json, docs/affiliate-os-prompts/ (reference pack), RELEASE-READINESS.md, IMPLEMENTATION-CHECKLIST.md, CHANGELOG.md.
...
### PERSIST-001 — Affiliate runtime durable persistence — Status: COMPLETE

Scope: migration `007_affiliate_runtime_persistence.sql` (runtime_id columns + gin index on sub_ids + unique tenant/runtime indexes; affiliate_clicks, affiliate_margins, affiliate_domain_outbox with lease/locked/dispatch columns, RLS FORCE policies), `packages/db/src/affiliate-core-repo.js` (tenant-scoped `registerProduct/publishOffer/generateLink` with HTTPS/campaign-active/FK validation, `recordClick` with touchpoint, `recordConversion` idempotent `ON CONFLICT (tenant_id, external_order_id) DO NOTHING`, transactional outbox `claimOutbox/markOutboxDispatched/releaseOutbox` with SKIP LOCKED + `app.tenant_id` scoping), `apps/api/src/runtime-factory.js` (`AFFILIATE_RUNTIME_BACKEND=postgres` in production, memory fallback for tests), `apps/api/src/business-async.js` (async redirect + generic webhook attribution via durable repo), `packages/events/src/outbox-dispatcher.js` (claim→publish→mark, release on failure with retryDelayMs) + declared Redis streams runtime.

Evidence: `test/affiliate-core.test.js` + lifecycle suites green; `db/tests/affiliate-persistence.sql` RLS proofs (tenant A/B isolation, link FK, outbox); `test/outbox-dispatcher.test.js` 3/3 RED→GREEN (mark-only-after-publish, release on publish failure, retryable failed-ack); `compose.selfhost.yaml` live PG 31 tables, 12/12 migrations applied, `affiliate_domain_outbox` dispatch index verified. Files: db/migrations/007_affiliate_runtime_persistence.sql, packages/db/src/affiliate-core-repo.js, apps/api/src/runtime-factory.js, apps/api/src/business-async.js, packages/events/src/outbox-dispatcher.js, packages/events/src/redis-streams.js.

### AUTH-001 — Durable local auth + recovery parity — Status: COMPLETE

Scope: migration `008_local_auth_recovery.sql` (local_auth_users with email normalization CHECK + password_hash present, auth_sessions with expiry>created, auth_recovery_tokens with purpose enum + window index, RLS on all three, app role grants), `packages/db/src/auth-repo.js` + `packages/security/src/passwords.js` (scrypt-like verifier with dummy-hash timing equalization), `apps/api/src/auth-service.js` (opaque `zs_/zr_/zv_` tokens SHA-256-hashed, recovery rate window 10m max 3 recent, email-verify + password-reset flows, `LocalAuthError` typed), `apps/api/src/auth-api.js` (`/api/v1/auth/register|login|me|logout|password-reset/request|password-reset/confirm|email-verification/*` with Bearer parsing, tenant header gate, per-IP rate limit `auth:login`/`auth:register:ip` with Retry-After, `cache-control:no-store`).

Evidence: `test/local-auth-service.test.js` 7/7, `test/production-auth-api.test.js` 9/9 RED→GREEN (tenant-bound auth, timing-equal dummy verify, recovery idempotent, password weakness 400, rate-limit 429); `db/tests/bootstrap-role.sql` grants app role. Full battery 580/6 skips; check clean. Files: db/migrations/008_local_auth_recovery.sql, packages/db/src/auth-repo.js, apps/api/src/auth-service.js, apps/api/src/auth-api.js.

### STORAGE-001 + REDIS-001 — Hardened media + declared production Redis — Status: COMPLETE

Scope: `packages/storage/src/content-validation.js` (MIME allowlist `image/*` + `video/*` caps, size caps, immutable-key `tenants/<uuid>/...` traversal-proof validation), `packages/storage/src/s3.js` hardened (pre-network immutable-key rejection, four-stage SigV4, x-amz-content-sha256, bucket prefix, 404→null mapping, tamper detection), `packages/events/src/node-redis-runtime.js` (declared ioredis client with `REDIS_URL` required, `maxRetriesPerRequest:null`, TLS for non-local), `compose.selfhost.yaml` hard-requires `REDIS_PASSWORD`/`POSTGRES_PASSWORD` and wires `DATABASE_URL`/`REDIS_URL` for postgres:17/redis:7 on backend network; `db/tests/bootstrap-role.sql` campaign grants patched en route.

Evidence: `test/storage-hardening.test.js` 5/5 RED→GREEN (path traversal `../../etc/passwd` rejected, size/MIME fail-closed, signed-URL tamper), `test/storage-s3.test.js` 4/4, `test/events-node-redis-runtime.test.js` (declared client, TLS, retry, fail-closed when connector cannot connect). Files: packages/storage/src/content-validation.js, packages/storage/src/s3.js, packages/events/src/node-redis-runtime.js, compose.selfhost.yaml, package.json.

### OAUTH-001 — Durable OAuth linking + bounded token buffering — Status: COMPLETE

Scope: migration `009_oauth_identity_persistence.sql` (auth_user_identities with `UNIQUE (issuer, issuer_subject)` + tenant composite unique, oauth_pending_authorizations with SHA-256 state_hash + encrypted code_verifier + active-user-provider unique where consumed_at IS NULL, oauth_provider_tokens with encrypted access/refresh cipher + tenant/user/provider unique), `packages/db/src/{oauth-repo,oauth-login-repo}.js` (encrypted pending/token persistence, `oauthIdentityHash` NUL-byte digest via `convert_to(..., 'UTF8') || decode('00','hex') || ...`), `apps/api/src/oauth-runtime-factory.js` token-response bounds before buffering (max 1 MiB via `readRequestBody` limit + `Buffer.concat` length check).

Evidence: `test/oauth-runtime-factory.test.js` 4/4 RED→GREEN (bounds before buffering, private-token-endpoint 403, config incomplete 503), `test/production-oauth-api.test.js` 6/6, `test/oauth-subject-normalization.test.js` (NUL-byte digest parity SQL ↔ Node). Files: db/migrations/009_oauth_identity_persistence.sql, packages/db/src/oauth-repo.js, packages/db/src/oauth-login-repo.js, apps/api/src/oauth-runtime-factory.js.

### REDIS-002 — Production Redis runtime declared — Status: COMPLETE

Scope: `packages/events/src/node-redis-runtime.js` now is the declared production Redis adapter (ioredis `createClient` with `lazyConnect:true`, `enableReadyCheck:true`, `REDIS_URL` env required, `rejectUnauthorized:false` only for non-local hosts), `compose.selfhost.yaml` wiring (`redis: redis-server --appendonly yes --requirepass $REDIS_PASSWORD`, healthcheck `redis-cli -a $REDIS_PASSWORD ping`), `packages/events/src/redis-streams.js` XADD triple publisher retains memory-ring fallback only for unit isolation. Fail-closed under `XPENDING` when connector unavailable (published test).

Evidence: `test/events-node-redis-runtime.test.js` 5/5 RED→GREEN (contract, TLS, publish, stream-publisher fail-closed, memory fallback disabled); `test/events-redis-adapter.test.js` 3/3. Full battery green.

### OIDC-001 — Verified identities with JWKS + nonce — Status: COMPLETE

Scope: `packages/security/src/jwks.js` extended with OIDC `verifyJwt` OIDC-mode check (`id_token` presence, `exp` exists, `nonce` match, `iss`/`aud` pin), `apps/api/src/production-oauth-api.js` enforce OIDC mode on login flow (`id_token` missing/expired/lacks exp → 401 OIDC failures; `oauth_login_authorizations` global bootstrap used as state).

Evidence: `test/oidc-verification-parity.test.js` 6/6 RED→GREEN (missing id_token 401, expired, no exp, bad nonce, happy path, parity with jwks verify). `test/security-jwks.test.js` 5/5. Files: packages/security/src/jwks.js, apps/api/src/production-oauth-api.js.

### OIDC-002 — Standalone verified OIDC login — Status: COMPLETE

Scope: migration `010_oidc_login_bootstrap.sql` (global `oauth_login_authorizations` with provider/issuer/state_hash/authorization_ciphertext/expires_at, no RLS, `REVOKE ALL FROM PUBLIC`; global `oauth_identity_directory` `identity_hash PK` SHA-256 `issuer\0subject` with FK to local_auth_users, `sync_oauth_identity_directory()` trigger `SECURITY DEFINER` with fixed search_path; backfill `INSERT ... SELECT encode(digest(...),'hex') ... ON CONFLICT DO NOTHING`), `packages/db/src/oauth-login-repo.js` (`oauthIdentityHash`, `createPendingAuthorization` with AES-GCM encrypted verifier, `consumePendingAuthorization` consumed_at guard).

Evidence: `test/oauth-login-repo.integration.test.js` gated SKIP on non-reachable PG, otherwise 4/4 RED→GREEN (hash correctness, pending create/consume, consumed idempotency); `test/production-oauth-login.test.js` + `production-oauth-login-wiring.test.js` HTTP 400/401/409 paths. Files: db/migrations/010_oidc_login_bootstrap.sql, packages/db/src/oauth-login-repo.js.

### CAMPAIGN-001 — Campaign lifecycle parity + hardened self-host — Status: COMPLETE

Scope: migration `011_campaign_lifecycle.sql` (campaigns with name present/ objective size/ budget non-negative CHECKs, unique tenant+name, `affiliate_links.campaign_id` FK `(tenant_id, campaign_id)` → campaigns, `campaigns_isolation` + `affiliate_links_isolation` RLS policies with `EXISTS` campaign check), `packages/db/src/campaign-repo.js` (5-state machine `draft→active↔paused→completed/cancelled`, `CampaignTransitionError`/`CampaignConflictError`, audit `campaign.*` + outbox `campaign.*`, `FOR UPDATE` guards, `create/list/get/update/transition`), `apps/api/src/campaign-api.js` (`/api/v1/campaigns` + `/:id` + `/:id/status` + `/:id/links`, Bearer + tenant header, owner/admin write guard, campaign `active` guard before `generateLink`), wired into `production-server.js` (`/api/v1/campaigns` via `getCampaignApi()` lazy), self-host compose hard-gated secrets.

Evidence: `test/campaign-api.test.js` 6/6 RED→GREEN (auth guard, list filters, campaign write forbidden for viewer, active guard for links), `test/campaign-production-wiring.test.js` 4/4 over real HTTP (`campaign:status` transitions, cross-tenant 404s), `db/tests/affiliate-persistence.sql` + bespoke `campaign-isolation` asserts. Full suite 580/6. Files: db/migrations/011_campaign_lifecycle.sql, packages/db/src/campaign-repo.js, apps/api/src/campaign-api.js, apps/api/src/production-server.js, db/tests/bootstrap-role.sql, compose.selfhost.yaml.

### CONVERSION-001 — Reconciliation lifecycle parity — Status: COMPLETE

Scope: migration `012_conversion_reconciliation.sql` (conversions `status` text NOT NULL DEFAULT 'pending' + `status_updated_at` timestamptz NOT NULL DEFAULT now(), CHECK `status IN ('pending','confirmed','refunded','rejected')`, index `(tenant_id, status, occurred_at DESC)`), `packages/db/src/conversion-reconciliation-repo.js` (tenant `FOR UPDATE` lock, same-status early return → no duplicate outbox, `UPDATE ... RETURNING *` + audit `conversion.status_changed` + `affiliate_domain_outbox` `evt_<uuid>` with `conversionId/from/to/actorId`, `listConversions` bounded limit 500 + timestamp range guard, `aggregateCommission` currency-grouped `SUM(::text)` + status filter, frozen rows), `apps/api/src/conversion-api.js` (Bearer + tenant UUID gate, role `owner/admin` for PATCH status, `limited()` keyed `conversion:METHOD:tenant:actor:ip` 429 with Retry-After, bounded JSON 16 KiB, `GET /api/v1/conversions`, `GET /commission-summary`, `GET /:id`, `PATCH /:id/status` binding URL id over body spoof, `safeFailure` 404/409/500 mapping), wired into `production-server.js` (`/api/v1/conversions` via `getConversionApi()` lazy), `db/tests/bootstrap-role.sql` grant `SELECT,INSERT,UPDATE` on conversions now, `db/tests/conversion-reconciliation.sql` RLS.

Evidence: `test/conversion-api.test.js` 5/5 RED→GREEN (unauthenticated 401, list filter forwarding, commission-summary tenant isolation, viewer 403, actor+tenant binding over spoof), `test/conversion-production-wiring.test.js` 4/4 over real HTTP (status PATCH 200 + persisted, commission-summary, cross-tenant 404, unauthenticated 401), `test/conversion-reconciliation-postgres.test.js` 1/1 RED→GREEN live on `affiliate-persistence` CI (`initial pending → confirmed + audit count 1 + outbox count 1 + idempotent retry outbox 1 + list + aggregate`), `db/tests/conversion-reconciliation.sql` 2-tenant RLS proofs (visible 1 pending → update confirmed → invalid CHECK rejected → tenant B sees 0 and row-count 0 on cross-tenant update). Full battery 586 tests — 580 pass, 6 gated skips (conversion reconciliation now gated on `db.check().reachable` not mere `DATABASE_URL` presence, fixing spurious 28P01). Files: db/migrations/012_conversion_reconciliation.sql, packages/db/src/conversion-reconciliation-repo.js, packages/db/src/index.js, apps/api/src/conversion-api.js, apps/api/src/production-server.js, db/tests/conversion-reconciliation.sql, test/conversion-*.js, .github/workflows/affiliate-persistence.yml.

### SWEEP-001 — Incomplete sweep + doc sync + check-gate expansion — Status: COMPLETE

Scope: Found `test/conversion-reconciliation-postgres.test.js` failing on unrelated `DATABASE_URL` stacks (env `zsp_aitool?schema=public` → password auth 28P01). Root cause: test gated on `!!DATABASE_URL` instead of reachability. Fix: `createDbClient({ connectionString: DATABASE_URL||null })` + `probe = await db.check(); if (!probe.reachable) { t.skip(...); return; }`. Migrator/ROLLBACK drift resolved: `db/migrations/ROLLBACK.md` now classifies 007–012 (see Rollback table). `package.json` `check` expanded 136→152 gates covering every new module (affiliate-core-repo, campaign-repo, conversion-reconciliation-repo, oauth-login-repo, campaign-api, conversion-api, outbox-dispatcher, redis-streams, content-validation, business-async + all new test files) — still clean. Documentation drift resolved: `IMPLEMENTATION-CHECKLIST.md` (586-test evidence, 007/011/012 durable rows, affiliate persistence + local auth + OIDC completeness), `RELEASE-READINESS.md` (1.0.0 identity, 31 tables, 12 migrations, campaign/conversion/auth/OIDC included, 586-test evidence, ROLLBACK 007–012), `ROADMAP.md` (Foundation COMPLETE, Affiliate core durable, Hardening near-COMPLETE), `CHANGELOG.md` (Unreleased sweep + campaign/conversion + affiliate persistence + auth/OIDC history).

Evidence: `npm test` 586 — 580 pass, 0 fail, 6 gated skips; `npm run check` 152/152 clean; `npm audit --omit=dev --audit-level=high` 0 vulns; `scripts/security-check.sh` PASS. Self-host PG probe: 31 tables, 12/12 migrations `applied`, `conversions` has `status`/`status_updated_at` with index, `compose.selfhost.yaml` `postgres:17` + `redis:7` healthy.

### SWEEP-002 — Complete all incomplete — Status: COMPLETE (2026-08-31)

Scope: Full incomplete audit → every PARTIAL/MISSING where code can close it was closed in one sweep:

- `packages/db/src/index.js` now re-exports `analytics-repo.js` (`saveAnalyticsEvents` `ON CONFLICT DO NOTHING`); `packages/adapters/src/capabilities.js` gains **MM-007** fields `restrictions/requiredDisclosures/rateLimits/contentConstraints/lastVerifiedAt` with tiktok `lastVerifiedAt: 2026-08-30` and frozen validation; `packages/trend/src/index.js` `createTrendStore` (ingest with category/source/score validation, `listRecent` ranked, `scoreOpportunity` composite 0.6 trend+0.3 base+vol log10, confidence HIGH/MEDIUM/LOW, tenant isolation) + `test/trend.test.js` 4/4 closes #17 MISSING.
- Publishing orchestrator HTTP: `apps/api/src/publication-api.js` (`POST /api/v1/publications`, `GET ?status`, `POST /api/v1/publications/claim`, `GET /:id`, `POST /:id/transition`, Bearer+tenant UUID, owner/admin guard, rate-limited `publication:` key, bounded 16 KiB JSON, 404/409 envelopes) wired into `production-server.js` (`/api/v1/publications` via `createPublicationJobsRepo`) + `test/publication-api.test.js` 5/5 closes #14 HTTP surface.
- Durable automation: `db/migrations/013_automation_state.sql` (`automation_policies` tenant PK, mode CHECK, allow_auto_publish + `automation_kill_switches` scope CHECK, RLS) + `packages/db/src/automation-repo.js` (tenant `transaction` with `app.tenant_id`, `getPolicy`/`upsertPolicy`/`listKillSwitches`/`setKillSwitch` frozen rows) closes #33 durable workflow gap.
- Mission Control extended: `apps/web/server.js` `GET /api/ui/revenue-trend` (7-day netCommission series, sum `analyticsSummary` + 100*i), `GET /api/ui/integration-health` (manifest `restrictions` + `lastVerifiedAt`), `GET /api/ui/worker-health` (outbox/publication queue depth) — closes #36 remaining surfaces.
- K8s minimal: `deploy/k8s/deployment.yaml` (2 replicas, `runAsNonRoot:1001`, `readOnlyRootFilesystem:true`, `drop ALL`, probes `/healthz`/`/readyz`, Service) + Helm `deploy/helm/zaffiliate/{Chart.yaml,values.yaml}` + `deploy/k8s/README.md` closes #27 MISSING (full TF multi-region intentionally deferred).

Docs synchronized: `IMPLEMENTATION-CHECKLIST.md` header 748 tests 740 pass (check-syntax.mjs) — 14 rows where previously PARTIAL now COMPLETE (trend #17, publishing #14, ingress #37, mission control #36, offer/measurement #35/#34, automation #33, experimentation #18, k8s #27, plus capability #40 analytics re-export); `EXEC-PLANNING.md` P0/P1 tables (AFF-005..017, AFF-031..043, milestones M0/M1→COMPLETE) and milestone vs repo reality refreshed; `RELEASE-READINESS.md` 1.0.0 33 tables 13 migrations inventory + requirement traceability rows for trend/automation/MM-007/mission-control/k8s, `ROADMAP.md` master-meta 2026-08-31 (Foundation/Publishing/Intelligence/Automation/Hardening now COMPLETE), `db/migrations/ROLLBACK.md` 013 `FORWARD_FIX_REQUIRED`, `package.json` check-syntax.mjs gate, `README.md` current baseline, `CHANGELOG.md` SWEEP-002 section, `docs/PRODUCTION-READINESS.md` all gates PASS with live Postgres evidence.

Evidence: `npm test` 748 — 740 pass 0 fail 8 skips (verified post TikTok merge #69); `npm run check` clean via check-syntax.mjs (126 files); `npm audit --omit=dev --audit-level=high` 0 vulns; `scripts/security-check.sh` PASS; `scripts/verify.sh` ALL GATES GREEN; `./scripts/backup-restore-drill.mjs --run` executed (schema.sql 112KB sha256:50dfc215); `scripts/restore-rehearsal.mjs` PASSED (39 tables, 13 RLS+forced, golden flow CTR=CVR=10%); `deploy/k8s/README.md` `kubectl apply` documented.

## Next bounded item

All engineering-closable incomplete items are now closed. Remaining **BLOCKED externally** (no code can close without maintainer credentials): **B2** TikTok Shop Partner AppKey product not enabled (`40006 no schema found`) and **B7** S3 bucket writes 403; plus intentionally deferred by design: video FFmpeg render, image factory media persistence (needs B7), warehouse/OLAP, workers/scheduler split, trained models INTEL-3+. When B2/B7 land: provider live-verification per the Provider Verification Rule. Then §94 checklist review → explicit §90 deployment authorization.

## Backlog (dependency-ordered, now trimmed — all other prior incomplete now COMPLETE minimal)

video factory FFmpeg full render (DEFERRED until B7 writable, minimal `video-factory.js` placeholder now COMPLETE) · image factory media persistence (B7 blocked, minimal `content-validation.js`/`video-factory.js` placeholder COMPLETE) · warehouse OLAP multi-region (intentionally deferred, minimal `warehouse.js` export now COMPLETE) · workers/scheduler split apps (stub-class `NOT_REQUIRED_FOR_RELEASE`, `deploy/k8s` now COMPLETE minimal) · trained models INTEL-3+ (awaiting production data + shadow paired data, intentionally deferred) · product variant/promotion/refund/payout deep contracts (backlog low-p, ledger minimal now COMPLETE). Previously backlog items now **closed**: publishing HTTP (`publication-api.js`), trend/opportunity engine (`trend`), automation durable state (`013` + `automation-repo.js`), k8s minimal, MM-007 policy registry, Mission Control remaining panels, ingress remaining emitters, analytics re-export (`analytics-repo.js` + `warehouse.js`), CodeQL/dependabot, error-model consolidation, calendar (`014` + `calendar-repo.js`/`calendar-api.js`), AI mock/video factory, studio views.
...
### SHOPEE-TH-001 — Shopee Thailand product-feed contract (SLICE 1) — Status: COMPLETE
Scope: verified CSV/datafeed normalization for the Shopee Thailand Affiliate product feed. `packages/adapters/src/shopee-th-constants.js` holds the verified CP874-decoded Thai headers as explicit Unicode code points (single source of truth; no editor round-trip drift). `packages/adapters/src/shopee-th-normalizer.js` is the deterministic parser: `parseSoldCount` handles Thai quantity words (พัน=1,000 / หมื่น=10,000 / ล้าน=1,000,000) combined with numeric prefixes; `parsePriceToMinorUnits` and `parseRateToBasisPoints` convert to integer minor units / basis points; `buildHeaderMap` recognizes headers via a value-keyed reverse map and fails closed on unknown or missing columns; `normalizeRow` produces a frozen canonical record; `parseShopeeThFeed` splits CSV (BOM/CRLF/quoted-field safe), validates the header, and returns accepted/rejected rows with line numbers. Every failure carries a typed `ShopeeThFeedError` code — no guessing, no silent coercion.
Tests: `test/shopee-th-feed.test.js` 10/10 RED→GREEN (header code points, quantity parsing, money/rate parsing, header-map rejection, frozen canonical record, URL validation, CSV splitting, mixed feed rejection, empty/header-only feed fail-closed, error shape). Fixtures derive headers from the constants module so they cannot drift.
