...
Files added: `db/migrations/016_tiktok_developer_accounts.sql`, `packages/db/src/tiktok-accounts-repo.js`, `packages/tiktok-developer/src/{auth,token-service,index}.js`, `packages/adapters/src/tiktok-publisher.js`, `packages/workflow/src/tiktok-publish-worker.js`, `packages/ai-content/src/tiktok-showcase.js`, `apps/api/src/tiktok-api.js`, `test/tiktok-{accounts-repo,developer-auth,publisher,publish-worker,showcase,api}.test.js`. Modified: `packages/db/src/index.js`, `apps/api/src/production-server.js`, `db/migrations/ROLLBACK.md`, `.env.example`, `CHANGELOG.md`.
