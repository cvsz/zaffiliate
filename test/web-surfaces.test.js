...
async function mutationHeaders(base, tenant) {
  const response = await fetch(`${base}/api/csrf-token`, { headers: { 'x-tenant-id': tenant } });
  assert.equal(response.status, 200);
  const body = await response.json();
  assert.equal(typeof body.token, 'string');
  assert.ok(body.token.length > 0);
  return { 'x-tenant-id': tenant, 'x-csrf-token': body.token, 'content-type': 'application/json' };
}
...
function assertSecurityHeaders(headers) {
  ...
  for (const directive of requiredCspDirectives) assert.ok(csp.includes(directive), `CSP missing directive: ${directive}`);
...
function assertOnlyReferenceSecrets(value) {
  if (typeof value === 'string') {
    if (/vault|credential|password|bearer |sk-/i.test(value)) assert.match(value, /^ref:/, `secret-shaped value must be a ref: pointer, got: ${value}`);
    return;
  }
  ...
  if (value !== null && typeof value === 'object') for (const item of Object.values(value)) assertOnlyReferenceSecrets(item);
}
...
test('GET / serves the shell HTML with CSP and every security header', async () => {
  await withServer(async (base) => {
    ...
    for (const asset of ['/app.js', '/views.js', '/styles.css']) {
      ...
      if (asset === '/styles.css') assert.match(assetResponse.headers.get('content-type') ?? '', /text\/css/);
      else assert.match(assetResponse.headers.get('content-type') ?? '', /javascript/);
    }
  });
});
...
test('encoded traversal, backslashes and absolute paths are rejected on static serving', async () => {
  await withServer(async (base) => {
    const probes = ['/%2e%2e/%2e%2e/etc/passwd','/..%2f..%2fetc%2fpasswd','/static/%2e%2e/%2e%2e/%2e%2e/etc/shadow','/..%5c..%5cetc%5cpasswd','/..%5cserver.js','//etc/passwd','/etc/passwd','/index.html%00.js'];
...
const apiGetEndpoints = ['/api/navigation','/api/audit','/api/billing/summary','/api/workflow/pending-approvals','/api/outreach/attempts','/api/analytics/funnel'];
...
test('every API endpoint returns 400 without x-tenant-id', async () => {
  await withServer(async (base) => {
    ...
    const postWithoutTenant = await fetch(`${base}/api/workflow/approve`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ approvalId: 'apr-1001', decision: 'approve' }) });
...
test('each fixture GET endpoint returns its expected shape', async () => {
  await withServer(async (base) => {
    ...
    for (const row of audit.rows) for (const key of ['id','at','actor','action','resource','outcome']) {
    ...
    for (const payload of [audit, billing, outreach, funnel]) assertOnlyReferenceSecrets(payload);
  });
});
...
test('approve happy path mutates fixture state and later GETs reflect the decision', async () => {
  await withServer(async (base) => {
    ...
    const decision = await fetch(`${base}/api/workflow/approve`, { method: 'POST', headers: await mutationHeaders(base, tenant), body: JSON.stringify({ approvalId: 'apr-1002', decision: 'approve' }) });
    ...
    const replay = await fetch(`${base}/api/workflow/approve`, { method: 'POST', headers: await mutationHeaders(base, tenant), body: JSON.stringify({ approvalId: 'apr-1002', decision: 'approve' }) });
    assert.equal(replay.status, 409);
    const rejection = await fetch(`${base}/api/workflow/approve`, { method: 'POST', headers: await mutationHeaders(base, tenant), body: JSON.stringify({ approvalId: 'apr-1003', decision: 'reject' }) });
...
test('approve decisions fail closed: invalid bodies 400, unknown id 404, state untouched', async () => {
  await withServer(async (base) => {
    const tenant = 'tenant-acme';
    const pendingBefore = await (await fetch(`${base}/api/workflow/pending-approvals`, { headers: { 'x-tenant-id': tenant } })).json();
    const invalidBodies = [
    ...
    for (const body of invalidBodies) {
      const response = await fetch(`${base}/api/workflow/approve`, { method: 'POST', headers: await mutationHeaders(base, tenant), body: JSON.stringify(body) });
      assert.equal(response.status, 400, `expected 400 for ${JSON.stringify(body)}`);
      assert.equal(typeof (await response.json()).error, 'string');
    }
    assert.equal((await fetch(`${base}/api/workflow/approve`, { method: 'POST', headers: await mutationHeaders(base, tenant), body: '{"approvalId":"apr-1001",' })).status, 400);
    assert.equal((await fetch(`${base}/api/workflow/approve`, { method: 'POST', headers: await mutationHeaders(base, tenant), body: '"just-a-string"' })).status, 400);
    const unknown = await fetch(`${base}/api/workflow/approve`, { method: 'POST', headers: await mutationHeaders(base, tenant), body: JSON.stringify({ approvalId: 'apr-9999', decision: 'approve' }) });
    assert.equal(unknown.status, 404);
    assert.deepEqual(await unknown.json(), { error: 'approval_not_found' });
    const pendingAfter = await (await fetch(`${base}/api/workflow/pending-approvals`, { headers: { 'x-tenant-id': tenant } })).json();
...
test('index.html is CSP-clean and links all control-plane surfaces', async () => {
  ...
  for (const tag of scriptTags) assert.match(tag, /\bsrc\s*=\s*"/i, `inline script without src violates CSP: ${tag}`);
  ...
  const navIds = ['overview','connections','products','campaigns','creators','links','content','publishing','outreach','workflows','analytics','commissions','billing','audit','security','admin'];
  for (const id of navIds) assert.match(html, new RegExp(`href="#${id}"`), `nav missing surface #${id}`);
  for (const label of ['Approval Center','Outreach Center','Attribution Funnel','Commissions','Billing &amp; Usage','Audit Log','Operator Console']) assert.ok(html.includes(label), `nav missing label ${label}`);
});